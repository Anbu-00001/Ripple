...
- **Code review only sees the diff.** A one-line change to a shared helper can break code *three files away* that no reviewer looked at. GitLab's own Orbit cookbook frames the question ("*what breaks if I change this service?*") — but a single query answers it only to a bounded depth.
